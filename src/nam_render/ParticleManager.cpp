...
namespace nam
{
    ...
    void ParticleManager::Update(ID3D12GraphicsCommandList* cmdList)
    {
        ...
        cmdList->SetPipelineState(Render->GetPsoManager()->GetPSO(PsoType::ParticleUpdate));
        Render->SetComputeRootSignature(RootSignatureName::_RootUpdateParticle);

        cmdList->SetComputeRootConstantBufferView(
            0,
            Render->GetConstantBuffersDatas()->GetGPUVirtualAdressOf(CBufferType::_ViewProj)
        );

        D3D12_GPU_DESCRIPTOR_HANDLE uavHandles[3];
        uavHandles[0] = Render->GetDescriptorHeapsWrapper()->GetUAVHandle(PARTICLE_UAV_SLOT);
        uavHandles[1] = Render->GetDescriptorHeapsWrapper()->GetUAVHandle(PARTICLE_FREELIST_SLOT);
        uavHandles[2] = Render->GetDescriptorHeapsWrapper()->GetUAVHandle(PARTICLE_FREECOUNT_SLOT);
    ...
    void ParticleManager::FlushSpawns(ID3D12GraphicsCommandList* cmdList)
    {
        ...
        cmdList->SetPipelineState(Render->GetPsoManager()->GetPSO(PsoType::ParticleSpawn));
        Render->SetComputeRootSignature(RootSignatureName::_RootSpawnParticle);
