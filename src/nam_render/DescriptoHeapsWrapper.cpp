...
namespace nam
{
	...
	{
		ID3D12Device* p_device = Render->GetDevice();

		D3D12_DESCRIPTOR_HEAP_DESC rtvHeapDesc;
		rtvHeapDesc.NumDescriptors = Render->GetSwapChainManager()->SwapChainBufferCount;
	...
	void DescriptorHeapsWrapper::SetDescriptorsHeaps()
	{
		...
		Render->GetCommandManager()->GetCommandList()->SetDescriptorHeaps(_countof(descriptorHeaps), descriptorHeaps);
	}
