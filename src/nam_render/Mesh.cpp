...
namespace nam
{
		...
		auto addFace = [&](XMFLOAT3 p0, XMFLOAT3 p1, XMFLOAT3 p2, XMFLOAT3 p3)
			{
				...
				vertices.push_back({ p0, color, {0.f, 1.f} });
				vertices.push_back({ p1, color, {0.f, 0.f} });
				vertices.push_back({ p2, color, {1.f, 0.f} });
				vertices.push_back({ p3, color, {1.f, 1.f} });
	...
	void Mesh::BuildCapsule(float radius, int stacks, int slices, float height, DirectX::XMFLOAT4 color, XMFLOAT3 offset, XMFLOAT3 ypr)
	{
		...
		float halfHeight = height * 0.5f;
		float totalHeight = height + 2.f * radius;
